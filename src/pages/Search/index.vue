...
export default {
  ...
  methods: {
    //把发请求的这个action封装到一个函数里面；将来需要再次发请求，你只需要在调用这个函数即可
    getData() {
    ...
    trademarkInfo(trademark) {  
      ...
      this.getData();       //再次发请求获取search模块列表数据进行展示
    },
    //删除品牌的信息
    removeTradeMark() {  
    ...
    //removeAttr删除售卖的属性
    removeAttr(index) {
      this.searchParams.props.splice(index, 1);   //再次整理参数
      this.getData();                             //再次发请求
    },
    //排序的操作
    changeOrder(flag) {
      ...
      let originOrder = this.searchParams.order;    /* originOrder为初始值 */
      let orginsFlag = originOrder.split(":")[0];   /* orginsFlag为初始的 点击综合or排序 */
